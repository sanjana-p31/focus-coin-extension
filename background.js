...
class FocusCoinEngine {
    constructor() {
        ...
        this.productiveSites = [
            ...
            'developer.mozilla.org', 'w3schools.com', 'freecodecamp.org', 'chatgpt.com'
        ];
    ...
    const result = await chrome.storage.local.get(['lastActiveDate', 'focusStreak', 'todayCoins']);
    ...
    if (result.lastActiveDate !== today) {
        ...
        if (result.lastActiveDate) {
            ...
            if (result.lastActiveDate === yesterday.toDateString()) {
                newStreak = (result.focusStreak || 0) + 1;
            }
        }
        ...
        console.log(`New day detected. Streak: ${newStreak}`);
    }
}
